...
export default function SystemLogsPage() {
  ...
  const fetchLogs = useCallback(async () => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ page, system: "true" });
  ...
  const hasFilters = filterAction !== "all" || filterEntity !== "all" || dateFrom || dateTo;
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="pt-2">
        ...
        <p className="text-sm text-muted-foreground">Các hành động tự động của hệ thống (không có người thực hiện)</p>
      </div>
              ...
              {logs.map((log) => {
                ...
                return (
                  <TableRow key={log.id}>
                    ...
                    <TableCell className="text-xs font-mono text-muted-foreground truncate max-w-[280px]">
                      {log.entity_id || "—"}
                    </TableCell>
